...
exports.getLimits = function getLimits(callback) {
  return errs.handle(
    errs.create({ message: "Joyent's API is not rate limited" }),
    callback
  );
};
...
exports.destroyServer = function destroyServer(server, callback) {
  ...
  this.getServer(serverId, function (err, server) {
    if (err) {
      return callback && callback(err);
    }

    self.stopServer(serverId, function (err) {
...
exports.rebootServer = function rebootServer(server, callback) {
  ...
  return this.request(rebootOptions, function (err, body, res) {
    return err
      ? callback(err)
